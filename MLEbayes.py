...
Xtrain=preprocessing.kag_X_train.values
Xtest=preprocessing.kag_X_test.values
...
def mlebayes():
    def gauss1(x):
        ...
        return (((-0.5*(x-m).T).dot(np.linalg.inv(S))).dot(x-m)) + np.log(P)+ (-0.5*len(x)*(log(2*np.pi))) - (0.5*log(det))
